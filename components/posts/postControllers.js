...
module.exports = {
	async getAllCurrentUserPost(req, res, next) {
		try {
			...
			if (req.user) {
				const posts = await db('posts').where({ user_id: req.user.id }).orderBy('id', 'asc')
	...
	async createNewPost(req, res, next) {
		const { type, post_url, user_id, title, description, thumbnail_url } = req.body
		if (type) {
			try {
				const newPost = {
					post_url,
		...
		if (req.user) {
			if (req.body.post_url) {
				try {
					const newUrl =
						req.body.post_url.indexOf('http') > -1 ? req.body.post_url : `http://${req.body.post_url}`
					...
					metadata.title === null ? (metadata.title = 'No title') : (metadata.title = metadata.title)
					metadata.image === null ? (metadata.image = null) : (metadata.image = metadata.image)
					try {
						const newPost = {
		...
		if (req.user) {
			db('posts')
				...
				.then((deletedRecord) => {
					...
					res.status(200).json({ deletedRecord: deletedCollection, msg: 'succesful delete' })
				})
