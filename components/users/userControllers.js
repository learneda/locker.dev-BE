...
module.exports = {
	...
	async editProfile(req, res, next) {
		...
		if (user) {
			try {
				const postPromise = await db('users').where({ id: user }).update({
					profile_picture,
					display_name,
					username,
					bio,
	...
	async getUserDetailsById(req, res, next) {
		...
		try {
			const selectPromise = await db('users')
				.where({ id: id })
				.select('username', 'display_name', 'profile_picture', 'bio', 'location', 'website_url', 'created_at')
			const selectCountPromise = await db('posts').where({ user_id: id }).count().first()
	...
	async recommendedFollow(req, res, next) {
		...
		if (friendsId.length) {
			...
			for (let i = 0; i < friendsOfFriends.length; i++) {
				const friendOfFriendsDetails = await db('users').where('id', friendsOfFriends[i].followed_by_id).first()
