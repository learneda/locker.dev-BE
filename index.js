...
io.on('connection', (socket) => {
	...
	socket.on('join', async function(data) {
		...
		const online_user = await db('online_users').insert({ user_id: data.user_id, socket_id: socket.id })

		if (online_user) {
			console.log('what online_user returns', online_user)
			const notifications = await db('notifications').where({ read: false, user_id: data.user_id })
	...
	socket.on('like', (data) => {
		if (data.action === 'unlike') {
			...
			db('notifications')
				.insert({ user_id: data.postOwnerId, post_id: data.post_id, type: 'like', invoker: data.username })
				.then((res) => {
